...
function hero(){const id=CFG.identity;return `<section class="hero capture" data-capture="hero"><div class="hero-mark">${owl()}<div><div class="handle">${esc(id.handle)}</div><div class="wordmark">${esc(id.wordmark)}</div></div></div><div class="hello" aria-label="${esc(id.hero_lines.join(' '))}">${id.hero_lines.map((x,i)=>`<div class="hello-row" data-row="${i}"><span>${esc(x)}</span></div>`).join('')}</div><div class="console"><div class="console-label">profile.trace</div>${id.console.map((x,i)=>`<div class="console-line" data-line="${i}"><span>›</span>${esc(x)}</div>`).join('')}<i class="cursor"></i><nav class="global-links">${(CFG.global_links||[]).filter(x=>x.url).map(x=>`<a href="${esc(x.url)}">${esc(x.label)} <i>↗</i></a>`).join('')}</nav></div></section>`}
